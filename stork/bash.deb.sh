...
function die {
    ...
    echo -e "$prefix: ${bold}URL: https://gitlab.isc.org/isc-projects/stork/${reset}"
    echo -e "$prefix: "
...
function config_url {
    ...
    echo "https://dl.cloudsmith.io/public/isc/stork/config.deb.txt?$params" | sed 's/ /%20/g'
}
...
function import_gpg_key {
    local text="Importing 'isc/stork' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/isc-stork-archive-keyring.gpg"
    
    curl -1sLf "https://dl.cloudsmith.io/public/isc/stork/gpg.77F64EC28053D1FB.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/isc-stork.gpg
        chmod 644 /etc/apt/trusted.gpg.d/isc-stork.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/isc-stork.list"
    check_fetch_config

    local text="Installing 'isc/stork' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'isc/stork' repository ..."
echo
